...
export const generateCropInsights = (field, rainProbability = 0) => {
  ...
  if (soilMoisture < config.minMoisture) {
    if (rainProbability >= 70) {
      urgency = 'Medium';
      insights.push(
        `💧 **Soil moisture is Low (${soilMoisture}%)**: However, heavy rain is forecasted in Taguig City today (Probability: ${rainProbability}%). **Recommendation:** The 5V Water Pump remains OFF to conserve reservoir water and prevent waterlogging. Monitor local rainfall.`
      );
    } else {
      urgency = 'High';
      insights.push(
        `💧 **Soil moisture is Low (${soilMoisture}%)**: Ideal range is ${config.minMoisture}%–${config.maxMoisture}%. Under-watering slows Pechay/Tomato growth and stresses roots. **Action Required:** The 5V Water Pump has been automatically activated to irrigate the field until moisture reaches optimal thresholds.`
      );
    }
